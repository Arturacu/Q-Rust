//! Integration tests covering the parser→transpiler→simulator pipeline.
...
"#;
        let parsed = parse_qasm(qasm).expect("parse");
        assert_eq!(parsed.num_qubits, 1);
        assert_eq!(parsed.operations.len(), 1);
        if let Operation::Gate { name, .. } = &parsed.operations[0] {
    ...
    fn test_decomposition_integration() {
        ...
        "#;
        let circuit = parse_qasm(qasm).expect("parse");
        let d = decompose_basis(&circuit);
        for op in &d.operations {
            if let Operation::Gate { name, .. } = op {
                assert!(matches!(name, GateType::U | GateType::CX));
            }
        }
        assert_eq!(d.operations.len(), 6);
    }
}